...
#include "queuewidgetitemdelegate.h"
...
QueueWidgetItemDelegate::QueueWidgetItemDelegate(QObject *parent)
    : QStyledItemDelegate(parent)
{
    restylizeUi();
}

void QueueWidgetItemDelegate::restylizeUi()
{
    m_idleIcon = {};
    m_resumeIcon = {};
    m_pauseIcon = {};
    ...
    m_completedIcon.addPixmap(QIcon::fromTheme("queue-completed").pixmap(16),
                              QIcon::Normal,
                              QIcon::On);
}

void QueueWidgetItemDelegate::paint(QPainter *painter,
                                    ...
                                    const QModelIndex &index) const
{
    ...
    myOption.palette.setColor(QPalette::All,
                              QPalette::WindowText,
    ...
    if (index.column() == COL_0_FILE_NAME) {
        ...
        auto progress = index.data(QueueWidgetItem::ProgressRole).toInt();
        auto state = static_cast<AbstractDownloadItem::State>(
            index.data(QueueWidgetItem::StateRole).toInt());
...
QWidget *QueueWidgetItemDelegate::createEditor(QWidget *parent,
                                               const QStyleOptionViewItem &option,
                                               const QModelIndex &index) const
{
    Q_UNUSED(option)
...
void QueueWidgetItemDelegate::setEditorData(QWidget *editor, const QModelIndex &index) const
{
...
void QueueWidgetItemDelegate::updateEditorGeometry(QWidget *editor,
                                                   const QStyleOptionViewItem &option,
                                                   const QModelIndex & /*index*/) const
{
...
QColor QueueWidgetItemDelegate::stateColor(AbstractDownloadItem::State state) const
{
    switch (state) {
...
QIcon QueueWidgetItemDelegate::stateIcon(AbstractDownloadItem::State state) const
{
    switch (state) {
    case AbstractDownloadItem::Idle:
        return m_idleIcon;
