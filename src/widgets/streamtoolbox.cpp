...
void StreamToolBox::setAutoSubtitleHidden(bool hidden)
{
    ...
    for (auto i = 0; i < ui->subtitleLangComboBox->count(); ++i) {
...
{
    return origin.isValid() && origin.toString() == CAPTION_AUTOMATIC;
}
...
void StreamToolBox::setData(const StreamObject::Data &data)
{
    ...
    // Subtitles
    {
        ...
        for (auto subtitle : subtitles) {
            ...
            auto origin = subtitle.isAutomatic ? CAPTION_AUTOMATIC : CAPTION_DEFAULT;
...
void StreamToolBox::_q_onToolButtonReleased()
{
    ...
    if (button) {
        ...
        auto index = static_cast<int>(m_buttons.indexOf(button));
...
void StreamToolBox::_q_onToolButtonClosed()
{
    ...
    setCurrentIndex(static_cast<int>(last));
}
#endif
